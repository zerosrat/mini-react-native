...
JSCExecutor::JSCExecutor()
    : m_context(nullptr), m_globalObject(nullptr) {
    initializeJSContext();
}
...
void JSCExecutor::initializeJSContext() {
    ...
    std::cout << "[JSCExecutor] JavaScript context initialized successfully" << std::endl;
}
...
void JSCExecutor::setupConsole() {
    ...
    JSObjectRef logFunc = JSObjectMakeFunctionWithCallback(m_context, logName,
...
void JSCExecutor::installBridgeFunctions() {
    ...
    installGlobalFunction("__nativeFlushQueuedReactWork",
        [](JSContextRef ctx, JSObjectRef function, JSObjectRef thisObject,
           size_t argumentCount, const JSValueRef arguments[], JSValueRef* exception) -> JSValueRef {
    ...
    // 注入日志函数
    installGlobalFunction("__nativeLoggingHook",
        [](JSContextRef ctx, JSObjectRef function, JSObjectRef thisObject,
           size_t argumentCount, const JSValueRef arguments[], JSValueRef* exception) -> JSValueRef {
...
void JSCExecutor::loadApplicationScript(const std::string& script, const std::string& sourceURL) {
    ...
    JSStringRef sourceURLStr = sourceURL.empty() ? nullptr :
                              JSStringCreateWithUTF8CString(sourceURL.c_str());

    JSValueRef exception = nullptr;
    JSValueRef result = JSEvaluateScript(m_context, scriptStr, nullptr, sourceURLStr, 0, &exception);
...
void JSCExecutor::setJSExceptionHandler(std::function<void(const std::string&)> handler) {
    m_exceptionHandler = handler;
}

void JSCExecutor::callJSFunction(const std::string& module, const std::string& method, const std::string& arguments) {
    // TODO: 实现 JavaScript 函数调用
    // 避免未使用参数的警告
    (void)arguments;
    std::cout << "[JSCExecutor] Calling JS function: " << module << "." << method << std::endl;
}

void JSCExecutor::installGlobalFunction(const std::string& name, JSObjectCallAsFunctionCallback callback) {
    JSStringRef funcName = JSStringCreateWithUTF8CString(name.c_str());
    JSObjectRef func = JSObjectMakeFunctionWithCallback(m_context, funcName, callback);
...
std::string JSCExecutor::jsValueToString(JSValueRef value) {
    JSStringRef strRef = JSValueToStringCopy(m_context, value, nullptr);
    if (!strRef) return "";
