...
function genMethod(moduleID, methodID, type) {
  ...
  if (type === MethodType.PROMISE) {
    ...
    fn = function (...args) {
      ...
      if (hasErrorCallback) {
        invariant(hasSuccessCallback, 'Cannot have a non-function arg after a function arg.')
      }
      ...
      BatchedBridge.enqueueNativeCall(moduleID, methodID, actualArgs, onFail, onSuccess)
    }
  }
...
function genModule(config, moduleID) {
  ...
  if (methods) {
    methods.forEach((methodName, methodID) => {
      ...
      const methodType = isPromise ? MethodType.PROMISE : isSync ? MethodType.SYNC : MethodType.ASYNC
